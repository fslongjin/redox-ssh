...
build:
	RUSTFLAGS=$(RUSTFLAGS) cargo $(TOOLCHAIN) build --all
...
build-release:
	RUSTFLAGS=$(RUSTFLAGS) cargo $(TOOLCHAIN) build --release --all
...
.PHONY: install
install: 
	RUSTFLAGS=$(RUSTFLAGS) cargo $(TOOLCHAIN) install --path . --no-track --root $(INSTALL_DIR) --force  --all-features